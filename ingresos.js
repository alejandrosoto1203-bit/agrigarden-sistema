...
function renderizarTablaIngresos(datos) {
    ...
    const esExcluido = (i) => i.tipo === 'ABONO' || i.tipo === 'TRASPASO' || i.categoria === 'COBRANZA' || i.categoria === 'PRÉSTAMO' || (i.categoria && i.categoria.toUpperCase().includes('RENTA'));
...
async function actualizarCalculosKPIsIngresos(datos) {
    ...
    const ingresosHoyArr = datos.filter(i => i.created_at.includes(hoyStr) && i.tipo !== 'ABONO' && i.tipo !== 'TRASPASO' && i.categoria !== 'COBRANZA' && i.categoria !== 'PRÉSTAMO');
    ...
    const datosSinAbonos = datos.filter(i => i.tipo !== 'ABONO' && i.tipo !== 'TRASPASO' && i.categoria !== 'COBRANZA' && i.categoria !== 'PRÉSTAMO');
